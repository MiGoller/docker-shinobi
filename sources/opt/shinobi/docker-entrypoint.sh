...
if [ "${EMBEDDEDDB}" = "true" ] || [ "${EMBEDDEDDB}" = "TRUE" ]; then
    ...
    if [ "$AIO" = "true" ] && [ "${MYSQL_HOST}" = "localhost" ]; then
        echo "========================================================================="
        echo "-------------------------------------------------------------------------"
        echo "                  Preparing embedded MariaDB server"
        echo "-------------------------------------------------------------------------"

        DB_INITIAL_SETUP="n"

        if [ ! -f /var/lib/mysql/ibdata1 ]; then
            echo "- Initial MariaDB setup ..."
            mysql_install_db --user=mysql --datadir=/var/lib/mysql --silent
            DB_INITIAL_SETUP="y"

            echo "- Starting MariaDB daemon ..."
            /usr/bin/mysqld_safe --user=mysql --skip-name-resolve --skip-networking=0 &

            sleep 2s

            if [ -n "${MYSQL_HOST}" ]; then
                echo -n "- Waiting for connection to MariaDB server on $MYSQL_HOST ."
                while ! mysqladmin ping -h"$MYSQL_HOST" --port="$MYSQL_PORT" --silent; do
                    sleep 1
                    echo -n "."
                done
                echo " established."
            ...
            echo "- Creating user accounts (for initial setups only)..."
mysql -u root --password="" --silent <<-EOSQL
...
EOSQL
            ...
            # /usr/bin/mysqld_safe --user=mysql --skip-name-resolve --skip-networking=0 &
            mysqld --user=mysql --skip-name-resolve --skip-networking=0 &
        fi
